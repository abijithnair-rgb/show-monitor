...
--   • status: MAX(1, 0, NULL) = 1 — a series is successful if it was flagged
--     success in any snapshot. This matches the canonical SR query's series-grain
--     definition (COUNT DISTINCT series with status=1). The language-level SR is
--     computed separately in the language_success_rate CTE below.
cp AS (
  SELECT
    series_id,
