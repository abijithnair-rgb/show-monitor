...
import { num, pickv, addDays } from './format';
import { effExperimentalDecision } from './model';
import { windowedSuccessRate } from './metrics';
...
export function computeNseVerdict(record, show, hdcRows, eps, today) {
  ...
  // Success rate uses the canonical trailing-7-day SETTLED window anchored to the
  // REVIEW date: [review-10, review-4]. A video's success only settles ~4 days
  // after approval, so this is the settled cohort as of the review day (e.g. a
  // 27 Jun review → 17 Jun … 23 Jun). Stage 2 anchors to the extended review date.
  const srWindow = (rev) => {
    const r = String(rev || '').slice(0, 10);
    if (!r || !eps) return { pct: null, pass: 0, n: 0 };
    return windowedSuccessRate(eps, addDays(r, -10), addDays(r, -4));
  };
  const sr1 = srWindow(record?.review_date);
  const sr2 = extended ? srWindow(record?.review_date2) : { pct: null, pass: 0, n: 0 };
