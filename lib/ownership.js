...
// Cumulative produced count for a label_produce claim. The POC needs lead time
// to act, so counting starts from the "Actions to be taken by" date (action_date)
// — not pickup — and runs to the earlier of today / the review date. Falls back
// to the pickup date when no action_date is set (then they're effectively equal).
