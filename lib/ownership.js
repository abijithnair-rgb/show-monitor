...
// ---- Label target builder (the 3-box picker: band · direction · amount %) ----
...
export function makeLabelTarget(band, dir, pct) {
  return { kind: 'label_change', band, dir, pct: Number(pct) || 0 };
}
...
export function targetText(target) {
  ...
  switch (target.kind) {
    ...
    case 'label_change': return `${target.dir === 'dec' ? 'Decrease' : 'Increase'} ${target.band} by ${target.pct}%`;
