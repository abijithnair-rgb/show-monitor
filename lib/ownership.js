...
// Videos published over a frequency claim's evaluation window (cadence count).
export function frequencyForClaim(claim, hdcRows, today = todayStr()) {
  if (!claim || claim.target?.kind !== 'frequency') return null;
  const { from, to } = claimWindow(claim, today);
  return cumulativeVideoCount(hdcRows, claim.show_id, from, to);
}
...
export function targetText(target) {
  ...
  switch (target.kind) {
    ...
    case 'frequency':
      return `${target.op === 'lte' ? 'Down to ≤' : 'Up to ≥'} ${target.n} videos/wk by review`;
...
export function currentFor(claim, s, data, hdcIdx, fatIdx) {
  ...
  if (claim?.target?.kind === 'frequency') cur.frequency = frequencyForClaim(claim, data.hdcRows);
...
export function targetReached(target, snap, cur) {
  ...
  switch (target.kind) {
    ...
    case 'frequency': return v != null && (target.op === 'lte' ? v <= target.n : v >= target.n);
...
export function evalVerdict(claim, cur, today = todayStr()) {
  ...
  // Review-gated: "at most N" cap (label "at most" or frequency "down to") — the
  // count only rises, so fail early if already breached, else wait for review.
  if (t && (t.kind === 'label_produce' || t.kind === 'frequency') && t.op === 'lte') {
...
export function progressLine(claim, cur) {
  ...
  if (t.kind === 'frequency') {
    return `Videos published so far: ${cur?.frequency ?? 0} of ${t.op === 'lte' ? '≤' : '≥'} ${t.n} (from the action date to review)`;
  }
  if (t.kind === 'sr_gte') {
