...
// The latest day an HDC label is SETTLED. HDC is computed off each video's 24h
// view/completion window, which only closes (and the label finalises) ~2 days
// after approval — so as of `today`, labels are settled only through today-2.
// HDC/Label experiments must ignore anything approved after this day (the label
// is still provisional and would flip the result prematurely).
export function hdcSettledTo(today = todayStr()) {
  return addDays(today, -2);
}

// Count videos of `band` a show approved in [fromDate, toDate] (inclusive),
// straight from the raw HDC rows. NOTE: in the HDC dataset publish_date =
// DATE(approved_on), so this counts content APPROVED in the window — and the
// upper bound is clamped to the HDC-settled day (today-2) so only videos whose
// label is final are counted.
export function cumulativeLabelCount(hdcRows, showId, band, fromDate, toDate, today = todayStr()) {
  if (!hdcRows || !fromDate || !toDate) return 0;
  const settled = hdcSettledTo(today);
  const upper = toDate < settled ? toDate : settled; // never count provisional (unsettled) labels
  if (upper < fromDate) return 0;
  const sid = String(showId);
  let c = 0;
  for (const r of hdcRows) {
    if (String(r.show_id) !== sid) continue;
    if (String(r.Label || '').toUpperCase() !== band) continue;
    const pd = String(r.publish_date || '').slice(0, 10); // = approved date in the HDC dataset
    if (pd && pd >= fromDate && pd <= upper) c += 1;
  }
  return c;
}
...
// trailing-7-day settled SR (videos published D-10..D-4) anchored to the
// experiment's REVIEW date — so a 27 Jun review judges the 17 Jun … 23 Jun
// window — using the SAME definition used everywhere else. Falls back to the
// data's latest day when no review date is set. eps = the show's episode rows.
export function srWindowForClaim(claim, eps, fatRows) {
  if (!claim || claim.target?.kind !== 'sr_gte') return null;
  const rev = claim.review_date ? String(claim.review_date).slice(0, 10) : null;
  return successRate(eps, fatRows, rev);
}

// ---- Experiment CONSTRAINTS ----
// Constraints are extra conditions to MAINTAIN while chasing the main target,
// e.g. "keep supply ≥ 5" or "keep L5 ≤ 1". They are warning-only — a breached
// constraint shows a red chip but NEVER changes the reached/failed verdict.
// Shape: { metric: 'supply'|'success_rate'|'L0'..'L5', op: 'gte'|'lte', value }.
//   • success_rate → canonical trailing-7d SR (same as the SR target).
//   • supply / L0..L5 → counted over the action→review window (claimWindow).
export const CONSTRAINT_METRICS = [
  { key: 'supply', label: 'Supply', unit: '' },
  { key: 'success_rate', label: 'Success rate', unit: '%' },
  ...LABEL_BANDS.map((b) => ({ key: b, label: b, unit: '' })),
];
const CONSTRAINT_KEYS = new Set(CONSTRAINT_METRICS.map((m) => m.key));
export const isConstraintMetric = (k) => CONSTRAINT_KEYS.has(k);

export function makeConstraint(metric, op, value) {
  return { metric, op: op === 'lte' ? 'lte' : 'gte', value: Math.max(0, Math.round(Number(value) || 0)) };
}
export function constraintMet(c, v) {
  if (v == null) return false;
  return c.op === 'lte' ? v <= c.value : v >= c.value;
}
export function constraintLabel(c) {
  const m = CONSTRAINT_METRICS.find((x) => x.key === c.metric);
  const unit = m?.unit || '';
  return `${m?.label || c.metric} ${c.op === 'lte' ? '≤' : '≥'} ${c.value}${unit}`;
}

// Current value of one constraint for a per-show claim.
export function constraintCurrentValue(c, claim, eps, hdcRows, fatRows, today = todayStr()) {
  if (c.metric === 'success_rate') { const sr = successRate(eps, fatRows); return sr && sr.n ? sr.pct : null; }
  const { from, to } = claimWindow(claim, today);
  if (c.metric === 'supply') return cumulativeVideoCount(hdcRows, claim.show_id, from, to);
  if (LABEL_BANDS.includes(c.metric)) return cumulativeLabelCount(hdcRows, claim.show_id, c.metric, from, to);
  return null;
}

// Evaluate every constraint on a per-show claim → [{ c, value, met, label }].
export function evalConstraints(claim, s, data, hdcIdx, fatIdx, today = todayStr()) {
  ...
  const eps = fIdx?.get(String(claim.show_id))?.eps;
  return list.map((c) => {
    const value = constraintCurrentValue(c, claim, eps, data.hdcRows, data.fatRows, today);
    return { c, value, met: constraintMet(c, value), label: constraintLabel(c) };
  });
}

// Validate/normalise an incoming constraints array (used client + server-side).
export function cleanConstraints(arr) {
  if (!Array.isArray(arr)) return [];
  return arr
    .filter((c) => c && CONSTRAINT_KEYS.has(c.metric) && (c.op === 'gte' || c.op === 'lte') && Number.isFinite(Number(c.value)) && Number(c.value) >= 0)
    .map((c) => makeConstraint(c.metric, c.op, c.value));
}
