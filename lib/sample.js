...
export function sampleData() {
  ...
  // Daily audience by source — 30 days × 4 sources for a few shows, so the
  // Deep Dive "Daily audience by source" chart has something to draw.
  const aud = [];
  const audPlan = {
    103: { organic: 520, push: 180, moe: 90, whatsapp: 40 },   // Astro Daily — organic-led
    104: { organic: 300, push: 420, moe: 160, whatsapp: 30 },  // Startup Diaries — push-heavy
    105: { organic: 900, push: 240, moe: 120, whatsapp: 110 }, // Comedy Hour — big, balanced
    102: { organic: 210, push: 90, moe: 60, whatsapp: 20 },    // Ramayana Nightly
  };
  Object.entries(audPlan).forEach(([show, base]) => {
    for (let i = 29; i >= 1; i--) {
      ...
      Object.entries(base).forEach(([src, b]) => {
        const views = Math.max(0, Math.round(b * wobble * trend));
        if (views === 0) return;
        aud.push({ show_id: +show, date_, source_type: src, views, users: Math.max(1, Math.round(views * 0.62)) });
      });
    }
  });
  ...
  return { eval: evals, fat: fatExpanded, hdc, ts, meta, rca, aud };
}
