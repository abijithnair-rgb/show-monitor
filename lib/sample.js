...
export function sampleData() {
  ...
  const hdcPlan = {
    101: ['L3', 'L4', 'L5', 'L5'], 103: ['L5', 'L5', 'L5', 'L5', 'L2'], 104: ['L0', 'L1', 'L4'],
  ...
  // Next-day return rates by viewer recency (NURR/CURR/RURR/SURR), weekly:
  // one row per show × ref_day (7 ref days, D-2..D-8). Several Hindi shows so
  // the language-median overlay is meaningful.
  const ret = [];
  const retPlan = {
    103: { nurr: 18, curr: 61, rurr: 35, surr: 22, base: { nurr: 1200, curr: 640, rurr: 210, surr: 95 } }, // Astro Daily
    104: { nurr: 15, curr: 56, rurr: 29, surr: 19, base: { nurr: 980, curr: 520, rurr: 160, surr: 70 } },  // Startup Diaries
    105: { nurr: 21, curr: 66, rurr: 38, surr: 25, base: { nurr: 1500, curr: 900, rurr: 300, surr: 140 } }, // Comedy Hour
    102: { nurr: 13, curr: 49, rurr: 23, surr: 16, base: { nurr: 320, curr: 180, rurr: 60, surr: 25 } },    // Ramayana Nightly
    101: { nurr: 11, curr: 44, rurr: 20, surr: 14, base: { nurr: 260, curr: 150, rurr: 50, surr: 20 } },    // Mahabharata Tales
    106: { nurr: 16, curr: 52, rurr: 27, surr: 18, base: { nurr: 400, curr: 230, rurr: 80, surr: 35 } },    // History Untold
  };
  Object.entries(retPlan).forEach(([show, p]) => {
    for (let i = 8; i >= 2; i--) {
      const ref_day = addDays(today, -i);
      const w = 1 + ((i * 5 + (+show % 7)) % 9 - 4) / 50; // deterministic ±8% wobble
      const row = { show_id: +show, ref_day };
      let total = 0;
      ['nurr', 'curr', 'rurr', 'surr'].forEach((k) => {
        row[k + '_pct'] = Math.round(p[k] * w * 10) / 10;
        row[k + '_base'] = p.base[k];
        total += p.base[k];
      });
      row.total_active_on_ref = total;
      ret.push(row);
    }
  });

  const meta = [
    { show_id: 104, language: 'hi', show_name: 'Startup Diaries', show_type: 'experimental', state: 'active', category_id: 73, category_name: 'Business', bu_name: 'Income', show_manager: 'Abijith', designer_name: 'Arjun K', freq: 'WEEKLY', specific_freq: 'Mon | Thu' },
    { show_id: 636, language: 'ml', show_name: 'Bhagavad Gita', show_type: 'main', state: 'active', category_id: 2, category_name: 'Devotion', bu_name: 'Awareness', show_manager: 'Nancy', designer_name: 'Ravi M', freq: 'DAILY', specific_freq: 'Mon | Tue | Wed | Thu | Fri' },
    ...
    // show_manager assignments (active/experimental) for the Show Manager tab.
    { show_id: 101, language: 'hi', show_name: 'Mahabharata Tales', show_type: 'main', state: 'active', show_manager: 'Abijith' },
    { show_id: 102, language: 'hi', show_name: 'Ramayana Nightly', show_type: 'main', state: 'active', show_manager: 'Abijith' },
    { show_id: 103, language: 'hi', show_name: 'Astro Daily', show_type: 'main', state: 'active', show_manager: 'Nancy' },
    { show_id: 105, language: 'hi', show_name: 'Comedy Hour', show_type: 'main', state: 'active', show_manager: 'Nancy' },
    { show_id: 106, language: 'hi', show_name: 'History Untold', show_type: 'main', state: 'inactive', show_manager: 'Nancy' },
    // Experimental shows — incl. duplicate rows (null + real manager) to mirror
    // show_detail and exercise the merge in buildMetaIndex.
    { show_id: 109, language: 'hi', show_name: 'Video Editing Pro', show_type: 'experimental', state: 'active', show_manager: null },
    { show_id: 109, language: 'hi', show_name: 'Video Editing Pro', show_type: 'experimental', state: 'active', show_manager: 'Kartikey' },
    { show_id: 110, language: 'ml', show_name: 'Yoga Basics (exp)', show_type: 'experimental', state: 'active', show_manager: 'Sasi' },
    { show_id: 110, language: 'ml', show_name: 'Yoga Basics (exp)', show_type: 'experimental', state: 'active', show_manager: null },
  ];
  ...
  // ---- Hindi day-over-day HDC RCA detail (level='HDC_SERIES') ----
  // Day A = 06-12 (earlier), Day B = 06-13 (later). The p90 view threshold rises
  // 1200 -> 1500 yet L0 still climbs (3 -> 6) — a "stronger signal". Includes a
  // first-time L0 (Investing Basics on day B) for the "new" flag.
  const hdcSeriesRow = (o) => Object.assign({
    level: 'HDC_SERIES', report_date: '2026-06-15', hdc_report_date: '2026-06-12',
    hdc_language: 'hi', segment: 'Income', show_name: '—', show_manager: 'Unassigned', hdc_series_title: '—',
    hdc_views_24h: 0, hdc_p90: 1200, hdc_threshold_value: 1200,
    hdc_target_cr: 35, hdc_achieved_cr: 0, hdc_view_pass: 0, hdc_cr_pass: 0, l0: 0, hdc_first_l0_date: null,
  }, o);
  const S = (pubDate, thr, o) => hdcSeriesRow({ hdc_report_date: pubDate, hdc_p90: thr, hdc_threshold_value: thr, ...o });
  const hdcSeries = [
    // ----- Day A : 2026-06-12 (threshold 1200) -----  3 L0
    S('2026-06-12', 1200, { segment: 'Income', show_name: 'Stock Market Daily', show_manager: 'Dhananjay', hdc_series_title: 'Nifty Setup Today', hdc_views_24h: 1800, hdc_achieved_cr: 41, hdc_view_pass: 1, hdc_cr_pass: 1, l0: 1, hdc_first_l0_date: '2026-06-12' }),
    S('2026-06-12', 1200, { segment: 'Income', show_name: 'Stock Market Daily', show_manager: 'Dhananjay', hdc_series_title: 'Options 101', hdc_views_24h: 1400, hdc_achieved_cr: 38, hdc_view_pass: 1, hdc_cr_pass: 1, l0: 1, hdc_first_l0_date: '2026-06-12' }),
    S('2026-06-12', 1200, { segment: 'Skill', show_name: 'Editing Mastery', show_manager: 'Kartikey', hdc_series_title: 'Cut on Action', hdc_views_24h: 1300, hdc_achieved_cr: 36, hdc_view_pass: 1, hdc_cr_pass: 1, l0: 1, hdc_first_l0_date: '2026-06-12' }),
    S('2026-06-12', 1200, { segment: 'Income', show_name: 'Stock Market Daily', show_manager: 'Dhananjay', hdc_series_title: 'Intraday Myths', hdc_views_24h: 1500, hdc_achieved_cr: 28, hdc_view_pass: 1, hdc_cr_pass: 0, l0: 0 }),
    S('2026-06-12', 1200, { segment: 'Awareness', show_name: 'Vishnu Puran', show_manager: 'Nancy', hdc_series_title: 'Kurma Avatar', hdc_views_24h: 700, hdc_achieved_cr: 44, hdc_view_pass: 0, hdc_cr_pass: 1, l0: 0 }),
    S('2026-06-12', 1200, { segment: 'Awareness', show_name: 'Vishnu Puran', show_manager: 'Nancy', hdc_series_title: 'Matsya Avatar', hdc_views_24h: 400, hdc_achieved_cr: 20, hdc_view_pass: 0, hdc_cr_pass: 0, l0: 0 }),
    S('2026-06-12', 1200, { segment: 'Skill', show_name: 'Editing Mastery', show_manager: 'Kartikey', hdc_series_title: 'Color Grading', hdc_views_24h: 900, hdc_achieved_cr: 30, hdc_view_pass: 0, hdc_cr_pass: 0, l0: 0 }),
    // ----- Day B : 2026-06-13 (threshold rose to 1500) -----  6 L0
    S('2026-06-13', 1500, { segment: 'Income', show_name: 'Stock Market Daily', show_manager: 'Dhananjay', hdc_series_title: 'Nifty Setup Today', hdc_views_24h: 2100, hdc_achieved_cr: 43, hdc_view_pass: 1, hdc_cr_pass: 1, l0: 1, hdc_first_l0_date: '2026-06-12' }),
    S('2026-06-13', 1500, { segment: 'Income', show_name: 'Stock Market Daily', show_manager: 'Dhananjay', hdc_series_title: 'Swing Trades', hdc_views_24h: 1900, hdc_achieved_cr: 40, hdc_view_pass: 1, hdc_cr_pass: 1, l0: 1, hdc_first_l0_date: '2026-06-13' }),
    // first-time L0 — a show that never hit L0 before in the window:
    S('2026-06-13', 1500, { segment: 'Income', show_name: 'Investing Basics', show_manager: 'Surya', hdc_series_title: 'SIP vs Lumpsum', hdc_views_24h: 1700, hdc_achieved_cr: 37, hdc_view_pass: 1, hdc_cr_pass: 1, l0: 1, hdc_first_l0_date: '2026-06-13' }),
    S('2026-06-13', 1500, { segment: 'Skill', show_name: 'Editing Mastery', show_manager: 'Kartikey', hdc_series_title: 'Cut on Action 2', hdc_views_24h: 1600, hdc_achieved_cr: 39, hdc_view_pass: 1, hdc_cr_pass: 1, l0: 1, hdc_first_l0_date: '2026-06-12' }),
    S('2026-06-13', 1500, { segment: 'Skill', show_name: 'Editing Mastery', show_manager: 'Kartikey', hdc_series_title: 'Sound Design', hdc_views_24h: 1550, hdc_achieved_cr: 36, hdc_view_pass: 1, hdc_cr_pass: 1, l0: 1, hdc_first_l0_date: '2026-06-13' }),
    S('2026-06-13', 1500, { segment: 'Awareness', show_name: 'Vishnu Puran', show_manager: 'Nancy', hdc_series_title: 'Varaha Avatar', hdc_views_24h: 1650, hdc_achieved_cr: 41, hdc_view_pass: 1, hdc_cr_pass: 1, l0: 1, hdc_first_l0_date: '2026-06-13' }),
    S('2026-06-13', 1500, { segment: 'Income', show_name: 'Stock Market Daily', show_manager: 'Dhananjay', hdc_series_title: 'Stop Loss Rules', hdc_views_24h: 1600, hdc_achieved_cr: 30, hdc_view_pass: 1, hdc_cr_pass: 0, l0: 0 }),
    S('2026-06-13', 1500, { segment: 'Awareness', show_name: 'Vishnu Puran', show_manager: 'Nancy', hdc_series_title: 'Narasimha', hdc_views_24h: 800, hdc_achieved_cr: 45, hdc_view_pass: 0, hdc_cr_pass: 1, l0: 0 }),
  ];
  rca.push(...hdcSeries);

  // ---- TTMK day-over-day HDC RCA detail (level='HDC_SERIES', regional) ----
  // Same two days (06-12 → 06-13). Regional has no BU split (segment='Other');
  // per-language view caps are lower (te 500, ta 330, kn/ml 200). Each language
  // has a mix of L0/non-L0 and one first-time L0 (the "new" flag).
  const T = (lang, pubDate, thr, o) =>
    hdcSeriesRow({ hdc_language: lang, segment: 'Other', hdc_report_date: pubDate, hdc_p90: thr, hdc_threshold_value: thr, hdc_target_cr: 50, ...o });
  const ttmkSeries = [
    // ---- Tamil (cap 330) ----  day A 1 L0 → day B 2 L0
    T('ta', '2026-06-12', 330, { show_name: 'Tamil Tales', show_manager: 'Arjun', hdc_series_title: 'Chola Empire', hdc_views_24h: 520, hdc_achieved_cr: 55, hdc_view_pass: 1, hdc_cr_pass: 1, l0: 1, hdc_first_l0_date: '2026-06-12' }),
    T('ta', '2026-06-12', 330, { show_name: 'Tamil Tales', show_manager: 'Arjun', hdc_series_title: 'Sangam Era', hdc_views_24h: 280, hdc_achieved_cr: 40, hdc_view_pass: 0, hdc_cr_pass: 0, l0: 0 }),
    T('ta', '2026-06-13', 330, { show_name: 'Tamil Tales', show_manager: 'Arjun', hdc_series_title: 'Chola Empire 2', hdc_views_24h: 600, hdc_achieved_cr: 58, hdc_view_pass: 1, hdc_cr_pass: 1, l0: 1, hdc_first_l0_date: '2026-06-12' }),
    T('ta', '2026-06-13', 330, { show_name: 'Madras Money', show_manager: 'Priya', hdc_series_title: 'SIP Tamil', hdc_views_24h: 410, hdc_achieved_cr: 53, hdc_view_pass: 1, hdc_cr_pass: 1, l0: 1, hdc_first_l0_date: '2026-06-13' }),
    // ---- Telugu (cap 500) ----  day A 1 L0 → day B 1 L0 (CR slips)
    T('te', '2026-06-12', 500, { show_name: 'Telugu Charitra', show_manager: 'Kiran', hdc_series_title: 'Kakatiya Rule', hdc_views_24h: 720, hdc_achieved_cr: 57, hdc_view_pass: 1, hdc_cr_pass: 1, l0: 1, hdc_first_l0_date: '2026-06-12' }),
    T('te', '2026-06-12', 500, { show_name: 'Hyderabad Bizz', show_manager: 'Kiran', hdc_series_title: 'Startup Telugu', hdc_views_24h: 460, hdc_achieved_cr: 49, hdc_view_pass: 0, hdc_cr_pass: 0, l0: 0 }),
    T('te', '2026-06-13', 500, { show_name: 'Telugu Charitra', show_manager: 'Kiran', hdc_series_title: 'Kakatiya Rule 2', hdc_views_24h: 690, hdc_achieved_cr: 44, hdc_view_pass: 1, hdc_cr_pass: 0, l0: 0 }),
    T('te', '2026-06-13', 500, { show_name: 'Hyderabad Bizz', show_manager: 'Kiran', hdc_series_title: 'Mutual Funds', hdc_views_24h: 560, hdc_achieved_cr: 56, hdc_view_pass: 1, hdc_cr_pass: 1, l0: 1, hdc_first_l0_date: '2026-06-13' }),
    // ---- Malayalam (cap 200) ----  day A 0 L0 → day B 1 L0 (new conversion)
    T('ml', '2026-06-12', 200, { show_name: 'Kerala Katha', show_manager: 'Anand', hdc_series_title: 'Onam Story', hdc_views_24h: 180, hdc_achieved_cr: 48, hdc_view_pass: 0, hdc_cr_pass: 0, l0: 0 }),
    T('ml', '2026-06-13', 200, { show_name: 'Kerala Katha', show_manager: 'Anand', hdc_series_title: 'Onam Story 2', hdc_views_24h: 240, hdc_achieved_cr: 54, hdc_view_pass: 1, hdc_cr_pass: 1, l0: 1, hdc_first_l0_date: '2026-06-13' }),
    T('ml', '2026-06-13', 200, { show_name: 'Kochi Cash', show_manager: 'Reshma', hdc_series_title: 'Gold Savings', hdc_views_24h: 150, hdc_achieved_cr: 41, hdc_view_pass: 0, hdc_cr_pass: 0, l0: 0 }),
    // ---- Kannada (cap 200) ----  day A 1 L0 → day B 1 L0
    T('kn', '2026-06-12', 200, { show_name: 'Kannada Kathe', show_manager: 'Vinay', hdc_series_title: 'Hampi Glory', hdc_views_24h: 260, hdc_achieved_cr: 56, hdc_view_pass: 1, hdc_cr_pass: 1, l0: 1, hdc_first_l0_date: '2026-06-12' }),
    T('kn', '2026-06-13', 200, { show_name: 'Kannada Kathe', show_manager: 'Vinay', hdc_series_title: 'Hampi Glory 2', hdc_views_24h: 300, hdc_achieved_cr: 59, hdc_view_pass: 1, hdc_cr_pass: 1, l0: 1, hdc_first_l0_date: '2026-06-12' }),
    T('kn', '2026-06-13', 200, { show_name: 'Bengaluru Bucks', show_manager: 'Sneha', hdc_series_title: 'Tax Tips', hdc_views_24h: 170, hdc_achieved_cr: 38, hdc_view_pass: 0, hdc_cr_pass: 0, l0: 0 }),
  ];
  rca.push(...ttmkSeries);

  // Paid DAU per show × day for the last 7 displayed days (D-7..D-1), plus the
  // trailing-7d average. One row per (show, date).
  const dau = [];
  const dauPlan = { 103: 820, 104: 900, 105: 1300, 102: 210, 101: 180, 106: 430 };
  Object.entries(dauPlan).forEach(([show, base]) => {
    for (let i = 7; i >= 1; i--) {
      const date_ = addDays(today, -i);
      const wobble = 0.82 + ((i * 9 + (+show % 5)) % 13) / 30; // deterministic ~0.82–1.22
      const paid_users = Math.max(1, Math.round(base * wobble));
      // simple trailing-7d avg proxy = base (the long-run level)
      dau.push({ show_id: +show, date_, paid_users, paid_users_7d_avg: base });
    }
  });

  // Per-show success rate (settled series, active shows) — one row per show,
  // carrying language so the dashboard can roll up to language / BU / POC.
  const langsr = [
    { show_id: 101, language: 'hi', series: 18, successful: 11, failed: 7 },
    { show_id: 102, language: 'hi', series: 22, successful: 14, failed: 8 },
    { show_id: 103, language: 'hi', series: 30, successful: 21, failed: 9 },
    { show_id: 105, language: 'hi', series: 26, successful: 19, failed: 7 },
    { show_id: 106, language: 'hi', series: 14, successful: 8, failed: 6 },
    { show_id: 636, language: 'ml', series: 19, successful: 11, failed: 8 },
    { show_id: 110, language: 'ml', series: 12, successful: 7, failed: 5 },
  ];

  return { eval: evals, fat: fatExpanded, hdc, ts, meta, rca, aud, ret, dau, langsr };
}
