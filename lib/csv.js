...
import { EVAL_REQUIRED, FAT_REQUIRED, FAT_EITHER, HDC_REQUIRED, TS_REQUIRED, META_REQUIRED, AUDIENCE_REQUIRED, RETENTION_REQUIRED, DAU_REQUIRED, LANGSR_REQUIRED } from './constants';
...
export function parseCombined(file, cb) {
  Papa.parse(file, {
    ...
    complete: (res) => {
      ...
      const buckets = { eval: [], fatigue: [], hdc: [], timespent: [], meta: [], audience: [], retention: [], dau: [], langsr: [] };
      let badJson = 0;
      for (const r of rows) {
      ...
      const checks = [
        ...
        ['Time-spent', buckets.timespent, TS_REQUIRED, []],
        ['Show-metadata', buckets.meta, META_REQUIRED, []],
        ['Audience', buckets.audience, AUDIENCE_REQUIRED, []],
        ['Retention', buckets.retention, RETENTION_REQUIRED, []],
        ['Paid DAU', buckets.dau, DAU_REQUIRED, []],
        ['Language SR', buckets.langsr, LANGSR_REQUIRED, []],
      ];
      ...
      cb({
        ...
        fat: buckets.fatigue,
        hdc: buckets.hdc,
        ts: buckets.timespent,
        showmeta: buckets.meta,
        aud: buckets.audience,
        ret: buckets.retention,
        dau: buckets.dau,
        langsr: buckets.langsr,
        meta: {
          eval: mk(buckets.eval),
          fat: mk(buckets.fatigue),
          hdc: mk(buckets.hdc),
          ts: mk(buckets.timespent),
          showmeta: mk(buckets.meta),
          aud: mk(buckets.audience),
          ret: mk(buckets.retention),
          dau: mk(buckets.dau),
          langsr: mk(buckets.langsr),
        },
      });
    },
  });
