...
export function outcomeChip(o, status) {
  if (o === 'success' || status === 1) return `<span class="chip chip-green">✓ success</span>`;
  if (o === 'failed' || status === 0) return `<span class="chip chip-red">✗ failed</span>`;
...
export function last10Table(eps) {
  const sorted = [...eps]
    .sort((a, b) => {
      const da = new Date(a.approved_dt),
        db = new Date(b.approved_dt);
  ...
  const rows = sorted
    .map((e) => {
      ...
      const h123 = num(e.h123_unique_viewers);
      const ce = num(e.comp_eff_h123_pct);
      const cell = (v) => `<td style="color:${retClass(v)};font-weight:600">${v != null ? fmtPct(v, 0) : '—'}</td>`;
      const title = e.series_title || e.title || '—';
      return `<tr>
      <td>${fmtDate(e.approved_dt)}</td>
      <td title="${esc(title)}">${esc(title)}</td>
      <td>${d0 != null ? fmtNum(d0) : '—'}</td>
      <td>${h123 != null ? fmtNum(h123) : '—'}</td>
      <td>${outcomeChip(e.video_outcome, e.video_status)}</td>
