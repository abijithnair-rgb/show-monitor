...
import { toneCls } from './model';
import { tip } from './tips';
import { buildHdcIndex, hdcNoContentMsg } from './hdc';
import { successRate, langAvgFat, globalBars } from './metrics';
...
export function outcomeChip(o, status, verdict) {
  ...
  if (o === 'success' || status === 1 || v === 'PASS') return `<span class="chip chip-green">✓ success</span>`;
  if (o === 'failed' || status === 0 || v === 'FAIL') return `<span class="chip chip-red">✗ failed</span>`;
...
export function explorerRow(s, hdcRows) {
...
export function last10Table(eps) {
  ...
  const rows = sorted
    .map((e) => {
      const hook = num(e.hook_retention_pct) ?? num(e.hook_retention_h123_pct);
      const mid = num(e.mid_retention_pct) ?? num(e.mid_retention_h123_pct);
      const end = num(e.end_retention_pct) ?? num(e.end_retention_h123_pct);
      const d0 = num(e.d0_views) ?? num(e.d0_unique_viewers);
      // H123 views — authoritative source is the CMS snapshot (content_metrics_run_log_v2):
      // `views`/`starts`. Fall back to the raw-watching `h123_unique_viewers` for older CSVs.
      const h123 = num(pickv(e, 'h123_unique_viewers', 'views', 'starts'));
      // Completion efficiency H123 = completion ÷ target × 100. Use comp_eff_h123_pct if
      // present, else derive from the CMS completion_rate / targ_comp.
      let ce = num(e.comp_eff_h123_pct);
      if (ce == null) {
        const compRate = num(e.h123_completion_rate_pct) ?? num(e.completion_rate);
        const tc = num(e.targ_comp);
        if (compRate != null && tc) ce = Math.round((compRate / tc) * 100 * 100) / 100;
      }
      const cell = (v) => `<td style="color:${retClass(v)};font-weight:600">${v != null ? fmtPct(v, 0) : '—'}</td>`;
      const title = e.series_title || e.title || '—';
      // maturity flag: the CMS snapshot may be provisional (H1/H12) until H123 freezes.
      const tag = String(e.snapshot_tag || '').toUpperCase();
      const provisional = tag && tag !== 'H123';
      const h123Cell = h123 != null
        ? `${fmtNum(h123)}${provisional ? `<div class="hint" title="Latest snapshot is ${esc(tag)} — not yet frozen at H123">${esc(tag)} · provisional</div>` : ''}`
        : '—';
      return `<tr>
      ...
      ${cell(hook)}${cell(mid)}${cell(end)}
      <td>${failChip(e.failure_mode)}</td>
    </tr>`;
    })
    .join('');
  return `<div class="card p-4">
    ...
    <div class="hint mt-1">Retention coloured: <span style="color:#E63946">&lt;50</span> · <span style="color:#D97706">50–70</span> · <span style="color:#2A9D8F">≥70</span>.</div>
  </div>`;
}
