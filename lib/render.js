...
// `sr` = the show's last-7d settled-H123 success rate ({ pct, pass, n }) or null.
export function explorerRow(s, hdcRows, assign, sr) {
  ...
  const srCell = sr && sr.n
    ? `<span class="font-medium">${sr.pct}%</span><div class="hint">${sr.pass}/${sr.n} settled</div>`
...
export function last10Table(eps) {
  ...
  const rows = sorted
    .map((e) => {
      ...
      // Comp efficiency = the video's ACTUAL CMS completion % (content_performance
      // .completion_rate = completes ÷ starts) ÷ its target completion (targ_comp)
      // × 100 — the exact achievement-vs-target the CMS records. Falls back to the
      // watching-derived H123 completion only when the CMS figure isn't present
      // (data synced before this column was added). NOT clamped to the pass/fail
      // verdict — the CMS status flag settles on a different basis, so a video may
      // read just under 100% yet be marked successful (or just over yet failed).
      const tc = num(e.targ_comp);
      const cmsRate = num(e.cms_completion_rate_pct);
      let ce = (cmsRate != null && tc) ? Math.round((cmsRate / tc) * 100 * 100) / 100 : num(e.comp_eff_cms_pct);
      if (ce == null) {
        const compRate = num(e.h123_completion_rate_pct) ?? num(e.completion_rate);
        ce = (compRate != null && tc) ? Math.round((compRate / tc) * 100 * 100) / 100 : num(e.comp_eff_h123_pct);
      }
  ...
  return `<div class="card p-4">
    ...
    <div class="overflow-x-auto"><table class="data-table">
      <thead><tr>
        ...
        <th${tip('comp_eff')}>Comp eff (CMS)</th>
