...
export function last10Table(eps) {
  ...
  const rows = sorted
    .map((e) => {
      ...
      // Comp efficiency H123 = the video's ACTUAL H123 completion % ÷ its target
      // completion (targ_comp) × 100 — how close it got to the per-video bar.
      // Computed straight from the CMS completion figures so the displayed % is
      // the true achievement ratio. It is NOT clamped to the pass/fail verdict:
      // the CMS status flag decides on a different/earlier basis, so a video may
      // read just under 100% yet be marked successful (or just over yet failed) —
      // the column reports the real ratio rather than forcing it to 99/100.
      const compRate = num(e.h123_completion_rate_pct) ?? num(e.completion_rate);
      const tc = num(e.targ_comp);
      let ce = (compRate != null && tc) ? Math.round((compRate / tc) * 100 * 100) / 100 : num(e.comp_eff_h123_pct);
