...
export function last10Table(eps) {
  ...
  const rows = sorted
    .map((e) => {
      ...
      // Keep Comp eff consistent with the settled CMS verdict (100% = hit target).
      // The H123 completion snapshot can overshoot the target even when CMS marked
      // the video failed (it decides on a different/earlier basis), which made the
      // column read >100% on a failed row. Clamp the DISPLAYED value to the same
      // side of the 100% line as the verdict so the two columns never contradict.
      const st = num(e.video_status);
      if (ce != null && st === 0 && ce >= 100) ce = 99;
      else if (ce != null && st === 1 && ce < 100) ce = 100;
