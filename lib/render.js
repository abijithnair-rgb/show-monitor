...
// Per-episode failure mode from the SAME cumulative retention values and
// duration floors the table cells display, so the chip explains the colours.
// A checkpoint "fails" when its cumulative retention is below its floor (i.e.
// not solidly green — amber or red); the failing checkpoint with the biggest
// shortfall wins. All three at/above floor → OK. Returns null if floors or all
// retention values are unavailable (caller falls back to the SQL value).
...
export function explorerRow(s, hdcRows) {
  ...
  return `<tr class="row-clickable" data-show="${esc(s.id)}">
    ...
    <td>${s.meta?.show_manager ? esc(s.meta.show_manager) : '<span class="hint">—</span>'}</td>
