...
// Where the show's current contribution sits versus its own peer bars (retain /
// stop). Used ONLY to phrase verdict copy so it never claims a show is "between
// the stop and retain bars" when it actually clears retain — e.g. an experiment
// that meets the retain bar but is still CONTINUE (too few users to promote).
// Wording only: the band / verdict are unchanged.
function barPosition(cur) {
  const c = num(cur?.l3w_current_contrib_pct);
  if (c == null) return 'between';
  const ret = num(cur?.retain_above_users_contrib_pct);
  const stop = num(cur?.stop_below_users_contrib_pct);
  if (ret != null && c >= ret) return 'above';
  if (stop != null && c < stop) return 'below';
  return 'between';
}
function barClause(pos, cap = false) {
  const t = pos === 'above' ? 'above the retain bar'
    : pos === 'below' ? 'below the stop bar'
      : 'between the stop and retain bars';
  return cap ? t[0].toUpperCase() + t.slice(1) : t;
}

// Lifecycle-only call: used when there is no Fatigue lens data.
export function lifecycleOnlyCall(L, cur) {
  const pos = barPosition(cur);
  ...
  if (L.isExp) {
    ...
    return wrap('WATCH', 'grey', 'Keep watching', `Experiment is ${barClause(pos)} (lifecycle verdict only — no fatigue data).`, 11);
  }
  ...
  return wrap('WATCH', 'grey', 'Keep watching', `${barClause(pos, true)} (lifecycle verdict only — no fatigue data).`, 11);
}
...
export function reconcile(show) {
  ...
  if (src === 'eval' || F.band === 'na') {
    const r = lifecycleOnlyCall(L, cur);
  ...
  if (L.band === 'stop' && F.band === 'shutdown')
    ...
    r = wrap('WATCH_AND_FIX', 'amber', 'Watch & fix', `${barClause(pos, true)}${pos === 'above' ? ', but' : ';'} episodes show ${F.label.toLowerCase()}. Address the creative issue and re-check next week — it decides which way this tips.`, 10);
  else if (L.band === 'watch')
    r = wrap('WATCH', 'grey', 'Keep watching', `${barClause(pos, true)} with no decisive creative signal. Monitor the next 2 weeks of contribution % before acting.`, 11);
