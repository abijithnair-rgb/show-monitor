...
// Sources tracked in the daily audience chart, in draw order.
export const AUDIENCE_SOURCES = ['organic', 'push', 'moe', 'whatsapp'];

// Audience index: per show_id, a dense daily series by source for the multi-line
// chart. audRows are (show_id, date_, source_type, views, users). Returns
//   Map show_id -> { dates: [d0..dn], views: {src: [..aligned..]}, users: {src: [...]} }
// with every date present for every source (missing day → 0).
export function buildAudienceIndex(audRows) {
  ...
  byShow.forEach((rows, k) => {
    ...
    const views = {}, users = {};
    AUDIENCE_SOURCES.forEach((s) => { views[s] = new Array(dates.length).fill(0); users[s] = new Array(dates.length).fill(0); });
    rows.forEach((r) => {
      const i = dateIx.get(String(r.date_).slice(0, 10));
      const src = String(r.source_type || '').toLowerCase();
      if (i == null || !(src in views)) return;
      views[src][i] += num(r.views) || 0;
      users[src][i] += num(r.users) || 0;
    });
    idx.set(k, { dates, views, users });
  });
  return idx;
}
