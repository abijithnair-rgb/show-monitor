...
import { BU_BY_CATEGORY } from './constants';
...
export function buildModel(data) {
  ...
  keys.forEach((k) => {
    ...
    const bu = catId != null ? (BU_BY_CATEGORY[catId] || '') : '';
    const show = { id: k, source, title, language, category, bu, status: statusOf(e, f, { evalMeta, fatMeta }), eval: e || null, fat: f || null, life: lifeClass(e) };
