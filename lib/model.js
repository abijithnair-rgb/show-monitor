...
// Show-metadata index: one row per show keyed by show_id (state, BU, owner, designer, cadence).
export function buildMetaIndex(metaRows) {
  const idx = new Map();
  if (!metaRows) return idx;
  metaRows.forEach((r) => idx.set(String(r.show_id), r));
  return idx;
}
...
// band/score drive the cross-lens reconciliation (kept tied to the SQL action
// recommendation). label/tone are the LENS DISPLAY and describe the actual
// fatigue diagnosis, so a recommended action never masquerades as the lens.
export function fatClass(rec) {
  ...
  // SHUTDOWN_CANDIDATE is an action (comp-pass + 6-day-return based), not a
  // retention diagnosis — surfacing "Sustained comp+retention miss" in the lens
  // mislabels shows whose retention is actually fine (e.g. a top-tier improving
  // show). Re-label by the real diagnosis. The shutdown signal still reaches the
  // unified call via band/score (unchanged).
  if (act === 'SHUTDOWN_CANDIDATE') {
    const d = fatigueDiagnosis(mode, zone);
    m.label = d.label;
    m.tone = d.tone;
  } else if (act === 'HOLD' && mode === 'INSUFFICIENT_DATA') {
    // "Not enough recent episodes to diagnose" ≠ "drop-off healthy".
    m.label = 'Insufficient data';
    m.tone = 'grey';
  }
  return { ...m, mode, act, zone };
}
...
export function buildModel(data) {
  const { evalRows, fatRows, evalMeta, fatMeta, metaRows } = data || {};
  ...
  keys.forEach((k) => {
    ...
    const show = { id: k, source, title, language, category, bu, meta: m || null, status: statusOf(e, f, { evalMeta, fatMeta }), eval: e || null, fat: f || null, life: lifeClass(e) };
