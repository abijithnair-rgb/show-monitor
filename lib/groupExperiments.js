...
export function groupLiveSnapshot(shows, data, scope, scopeValue) {
  ...
  if (!sr && data?.fatRows) {
    ...
    sr = pooledCanonicalSR(shows, data.fatRows, dStr(l), dStr(u));
  }
...
export function groupCurrentFor(claim, shows, data) {
  ...
  if (t.kind === 'sr_gte') {
    // Canonical per-show SR dataset (exact at any scope) when present; else pool
    // the fatigue rows over the claim's action→review window.
    const sr = scopeSuccessRate(data, shows) || pooledCanonicalSR(shows, data?.fatRows, from, to);
