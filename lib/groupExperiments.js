...
const scopeField = (s, scope) =>
  scope === 'language' ? s.language
    : scope === 'bu' ? s.bu
      : scope === 'category' ? s.category
        : s.manager; // poc

// ---- Composite scopes (a base value, optionally bifurcated by language) ----
// Category AND Business-unit experiments can run pan-language ("Comedy",
// "Income" → that base across every language) or scoped to one language of that
// base ("Comedy::hi", "Income::te"). The base value and language are joined with
// "::". Single-field scopes (language / POC) are not composite.
export const COMPOSITE_SCOPES = new Set(['category', 'bu']);
export const isCompositeScope = (scope) => COMPOSITE_SCOPES.has(scope);

const CAT_LANG_SEP = '::';
export function parseScopeValue(value) {
  const v = String(value || '');
  const i = v.indexOf(CAT_LANG_SEP);
  if (i === -1) return { base: v, lang: null };
  return { base: v.slice(0, i), lang: v.slice(i + CAT_LANG_SEP.length) || null };
}
export const makeScopeValue = (base, lang) =>
  lang ? `${base}${CAT_LANG_SEP}${lang}` : String(base || '');

// Distinct base values for a composite scope, with a show count, sorted by label.
// When `lang` is given, only bases present in that language are counted.
export function scopeBaseValues(model, scope, lang) {
  const counts = new Map();
  (model || []).forEach((s) => {
    if (lang && (s.language || '') !== lang) return;
...
// Languages present for a given base value in a composite scope, with counts.
export function scopeBaseLanguages(model, scope, base) {
  const counts = new Map();
  (model || []).forEach((s) => {
    if ((scopeField(s, scope) || '') !== base) return;
    const lg = s.language || '';
    if (!lg) return;
...
// Human label for a scope value (languages get their full name; composite scopes
// show the language split when present).
export const scopeValueLabel = (scope, value) => {
  if (isCompositeScope(scope)) {
    const { base, lang } = parseScopeValue(value);
    return lang ? `${base} · ${LANG_NAMES[lang] || lang}` : `${base} (all languages)`;
  }
...
export function scopeShows(model, scope, value) {
  ...
  if (isCompositeScope(scope)) {
    const { base, lang } = parseScopeValue(value);
    return model.filter((s) => (scopeField(s, scope) || '') === base && (!lang || (s.language || '') === lang));
  }
...
// For composite scopes this is a flat list: each base's pan-language option
// followed by its per-language splits.
export function scopeOptions(model, scope) {
  if (isCompositeScope(scope)) {
    const out = [];
    scopeBaseValues(model, scope).forEach((b) => {
      out.push({ value: b.value, n: b.n, label: scopeValueLabel(scope, b.value) });
      scopeBaseLanguages(model, scope, b.value).forEach((lg) => {
        const v = makeScopeValue(b.value, lg.value);
        out.push({ value: v, n: lg.n, label: scopeValueLabel(scope, v) });
      });
    });
    return out;
  }
