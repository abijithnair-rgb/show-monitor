...
// Canonical language-level success rate. Prefers the dedicated `langsr` dataset
// (sql/success_rate.sql / combined.sql) which matches the team's source-of-truth
// SR query EXACTLY (active+live shows, distinct settled series in the published
// window, status=1 ÷ status=1+status=0). Falls back to the precomputed
// language_success_rate_pct carried on the fatigue rows. Returns { pct, pass, n }
// or null when neither source is present.
export function languageSuccessRate(data, language) {
  if (!data || !language) return null;
  const lr = (data.langsrRows || []).find((r) => String(r.language) === String(language));
  if (lr) {
    const pass = num(lr.successful) ?? 0;
    const failed = num(lr.failed) ?? 0;
    const n = pass + failed;
    return { pct: n ? Math.round((pass / n) * 100) : null, pass, n };
  }
  const row = (data.fatRows || []).find(
...
export function groupLiveSnapshot(shows, data, scope, scopeValue) {
  ...
  if (data?.fatRows) {
    ...
    const from = dStr(l), to = dStr(u);
    if (scope === 'language' && scopeValue) {
      sr = languageSuccessRate(data, scopeValue) || pooledCanonicalSR(shows, data.fatRows, from, to);
    } else {
      sr = pooledCanonicalSR(shows, data.fatRows, from, to);
    }
  }
  return {
    labels,
...
export function groupCurrentFor(claim, shows, data) {
  ...
  if (t.kind === 'sr_gte') {
    // Language scope → canonical precomputed per-language SR when available,
    // else the same pooled active-shows / distinct-series computation used for
    // BU / POC, scoped to the claim's action→review window.
    if (claim.scope === 'language') {
      const sr = languageSuccessRate(data, claim.scope_value)
        || pooledCanonicalSR(shows, data?.fatRows, from, to);
      return { successRate: sr && sr.n ? sr.pct : null, srWindow: sr };
    }
    const sr = pooledCanonicalSR(shows, data?.fatRows, from, to);
    return { successRate: sr && sr.n ? sr.pct : null, srWindow: sr };
  }
