...
import { dStr, LANG_NAMES, num } from './format';
import { buildHdcIndex } from './hdc';
import { windowedSuccessRate, fatRefToday } from './metrics';
import {
...
// The model field a scope groups on.
const scopeField = (s, scope) =>
  scope === 'language' ? s.language : scope === 'bu' ? s.bu : s.manager;

// Human label for a scope value (languages get their full name).
export const scopeValueLabel = (scope, value) =>
  scope === 'language' ? (LANG_NAMES[value] || value) : value;
...
// Canonical language-level success rate, precomputed in fatigue.sql
// (language_success_rate_pct: active+live shows, published in the settled window,
// status=1 ÷ status=1+status=0). Constant across a language's rows, so the first
// matching row carries it. Returns { pct, pass, n } or null when absent.
export function languageSuccessRate(fatRows, language) {
  if (!fatRows || !language) return null;
  const row = fatRows.find(
    (r) => r.language === language && num(r.language_success_rate_pct) != null
  );
  if (!row) return null;
  return {
    pct: num(row.language_success_rate_pct),
    pass: num(row.language_sr_pass) ?? 0,
    n: num(row.language_sr_n) ?? 0,
  };
}
...
export function groupLiveSnapshot(shows, data, scope, scopeValue) {
  ...
  // Success rate. Language scope → canonical precomputed per-language number
  // (matches the source-of-truth SR query). Other scopes → pooled over the
  // scope's shows across the [ref-10, ref-4] settled window.
  let sr = { pct: null, pass: 0, n: 0 };
  if (scope === 'language' && scopeValue) {
    sr = languageSuccessRate(data?.fatRows, scopeValue) || sr;
  } else if (data?.fatRows) {
    const ref = fatRefToday(data.fatRows);
    const u = new Date(ref); u.setDate(u.getDate() - 4);
    const l = new Date(ref); l.setDate(l.getDate() - 10);
    const eps = data.fatRows.filter((r) => idSet.has(String(r.show_id)));
    sr = windowedSuccessRate(eps, dStr(l), dStr(u));
  }
  return {
    labels,
...
// ---- Current aggregate for a running claim, over its action→review window ----
function scopeEps(ids, fatRows) {
  const set = new Set(ids.map(String));
  return (fatRows || []).filter((r) => set.has(String(r.show_id)));
}
...
export function groupCurrentFor(claim, shows, data) {
  ...
  if (t.kind === 'sr_gte') {
    // Language scope → canonical precomputed per-language SR (matches the metric
    // box and the source-of-truth query). Other scopes → pooled over the
    // claim's action→review window.
    if (claim.scope === 'language') {
      const sr = languageSuccessRate(data?.fatRows, claim.scope_value);
      return { successRate: sr ? sr.pct : null, srWindow: sr || { pct: null, pass: 0, n: 0 } };
    }
    const sr = windowedSuccessRate(scopeEps(ids, data.fatRows), from, to);
