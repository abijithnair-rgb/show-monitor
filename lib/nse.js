...
export async function fetchNse() {
  ...
  if (!res.ok) return { configured: false, nse: {}, nseHistory: {} };
  return res.json();
}
...
export const extendNse = (id, review_date2, extend_count, manager_remark) =>
  post({ op: 'extend', id, review_date2, extend_count, manager_remark });
export const concludeNse = (id, final_verdict, concluded_by) =>
  post({ op: 'conclude', id, final_verdict, concluded_by });
export const setNseShowId = (id, show_id) => post({ op: 'set_show_id', id, show_id });
export const setNseStatus = (id, exp_status) => post({ op: 'set_status', id, exp_status });
