// Day-over-day HDC RCA — pure aggregation helpers (no React, no DOM), so
// they're unit-testable. Input = per-series detail rows (level='HDC_SERIES'),
// already filtered to one language (or language group) by the caller. Each row:
//   { publish_date, language, bu, manager, show, series, views, p90, threshold,
...
export function normalizeSeriesRow(r) {
  return {
    ...
    language: String(r.hdc_language || r.language || 'hi').toLowerCase(),
