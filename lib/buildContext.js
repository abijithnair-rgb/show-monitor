...
function compose(data) {
  ...
  // DATA WINDOW — the exact dates the per-show metrics below cover, so the bot
  // can name the week and never refuse a weekly report "for lack of a date slice".
  const epDates = [];
  (fatRows || []).forEach((r) => { const d = String(r.approved_dt || r.publish_date || '').slice(0, 10); if (/^\d{4}-\d{2}-\d{2}$/.test(d)) epDates.push(d); });
  epDates.sort();
  const epWin = epDates.length ? `${epDates[0]} … ${epDates[epDates.length - 1]}` : 'n/a';
  const hWin = hdcIdx && hdcIdx.window ? `${hdcIdx.window.lower} … ${hdcIdx.window.upper}` : 'n/a';
  L.push('');
  L.push(`DATA WINDOW: per-show success-rate/outcome data covers episode approved dates ${epWin}; HDC labels cover ${hWin}. This snapshot IS the latest loaded week of data. When the user asks for "this week" or a recent week (e.g. "6–12 June"), treat the table below as that week, state the covered window once, and produce the report — do NOT refuse because there is no date-range filter.`);

  // FULL per-show table (active & experimental) — the backbone for weekly / tiered
  // reports. One compact line per show with every metric a weekly report needs.
  const fatScoreLineOf = (s) => num(fatIdx.get(s.id)?.show?.show_fatigue_score);
  const d0MoveOf = (s) => num(fatIdx.get(s.id)?.show?.show_d0_viewers_pct_change_4w);
  const showTableLine = (s) => {
    const ev = s.eval?.cur;
    const hd = hdcIdx?.get(s.id);
    const sr = fatIdx.get(s.id)?.eps ? successRate(fatIdx.get(s.id).eps, fatRows) : null;
    const b = [`"${s.title}" #${s.id} [${s.status}${s.category ? '/' + s.category : ''}]`];
    b.push(`SR ${sr && sr.n ? `${sr.pass}/${sr.n}=${sr.pct}%` : '—'}`);
    b.push(`HDC ${hd && hd.supply ? `${hd.hdc}/${hd.supply}=${hd.hdcRatePct}% avgLabel=${hd.avgLevel != null ? 'L' + hd.avgLevel.toFixed(2) : '—'}${hd.modeLabel ? ' mode ' + hd.modeLabel : ''}` : '—'}`);
    b.push(`users ${num(ev?.show_users) ?? '—'}`);
    const c0 = num(ev?.l3w_current_contrib_pct);
    const prev = [num(ev?.l3w_minus_1_contrib_pct), num(ev?.l3w_minus_2_contrib_pct), num(ev?.l3w_minus_3_contrib_pct)].filter((v) => v != null);
    b.push(`contrib ${c0 != null ? c0 + '%' : '—'}${prev.length ? ` (prev wks ${prev.map((v) => v + '%').join('/')})` : ''}`);
    const d0c = d0MoveOf(s);
    if (d0c != null) b.push(`D0-viewers Δ4w ${d0c >= 0 ? '+' : ''}${d0c}%`);
    const f = fatScoreLineOf(s);
    if (f != null) b.push(`fat ${f.toFixed(2)}`);
    b.push(`call ${ACTION_META[s.rec.key]?.label || s.rec.key}`);
    return '  - ' + b.join(' · ');
  };
  const byContrib = (a, b) => (num(b.eval?.cur?.l3w_current_contrib_pct) || 0) - (num(a.eval?.cur?.l3w_current_contrib_pct) || 0);
  L.push('');
  L.push('FULL SHOW TABLE (ALL active & experimental shows — success rate pass/total, HDC count & avg label, users, contribution % with prior-week trend = the per-show audience movement, D0-viewer 4-week movement, fatigue, unified call). Tier / group / filter directly from THIS table — it has every show, so you never need to say a show is missing:');
  langs.forEach((lang) => {
    const rows = active.filter((s) => s.language === lang).sort(byContrib);
    if (!rows.length) return;
    L.push(` ${LANG_NAMES[lang] || lang} (${rows.length} shows):`);
    if (lang === 'hi') {
      ['Awareness', 'Income', 'Skill'].forEach((bu) => {
        const br = rows.filter((s) => s.bu === bu);
        if (br.length) { L.push(`  BU ${bu}:`); br.forEach((s) => L.push(showTableLine(s))); }
      });
      const nb = rows.filter((s) => !['Awareness', 'Income', 'Skill'].includes(s.bu));
      if (nb.length) { L.push('  BU unmapped:'); nb.forEach((s) => L.push(showTableLine(s))); }
    } else {
      rows.forEach((s) => L.push(showTableLine(s)));
    }
  });
  ...
  let text = L.join('\n');
  // keep it bounded (~60 KB safety cap — the full per-show table needs the room).
