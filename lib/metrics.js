...
// Success rate from episode rows: settled-H123 window (today-10..today-4).
// Canonical definition (matches the content_performance SR query): SR =
// successful ÷ (successful + failed) over the DISTINCT settled series published
// in the window. A series counts ONLY when it has a settled success flag
// (video_status 1 or 0); status NULL = not yet evaluated and is excluded
// entirely — no CMS-verdict override and no completion-rate fallback. There is
// NO "last 7" cap: every settled series in the window counts.
export function successRate(eps, fatRows, refDate) {
  // The settled window is [asOf-10, asOf-4]. asOf defaults to the data's latest
  // day (fatRefToday) — the rolling "as of now" SR. Pass refDate (YYYY-MM-DD) to
  // anchor it to a specific day instead, e.g. an experiment's review date so a
  // 27 Jun review yields the 17 Jun … 23 Jun window.
  const ref = refDate ? new Date(String(refDate).slice(0, 10)) : fatRefToday(fatRows);
  const upper = new Date(ref);
  upper.setDate(upper.getDate() - 4);
  const lower = new Date(ref);
  lower.setDate(lower.getDate() - 10);
  // De-duplicate by series_id so each settled series is counted once.
  const seen = new Map();
  (eps || []).forEach((e) => {
    const st = num(e.video_status);
    if (st !== 0 && st !== 1) return; // exclude NULL/unevaluated
    const d = new Date(epDate(e));
    if (isNaN(d) || d < lower || d > upper) return;
    const key = e.series_id != null && e.series_id !== '' ? String(e.series_id) : `${e.series_title || ''}|${epDate(e)}`;
    if (!seen.has(key)) seen.set(key, st);
  });
  const n = seen.size;
  if (!n) return { pct: null, pass: 0, n: 0 };
  let pass = 0;
  seen.forEach((st) => { if (st === 1) pass += 1; });
  return { pct: Math.round((pass / n) * 100), pass, n };
}
