...
export const METRIC_TIPS = {
  ...
  success_rate: '% of successful videos among the last 7 videos with a settled success flag, approved between 4 and 10 days ago (older than the 72h H123 window + 1 buffer day). SR = status=1 ÷ (status=1 + status=0); videos whose content_performance.status is still NULL (not yet evaluated) are excluded entirely. Denominator capped at 7. Matches the Daily RCA success-rate definition.',
  ...
  failure: 'Per-episode failure mode read off the same cumulative retention and per-length floors shown in this table: the checkpoint with the biggest shortfall below its floor (HOOK / PACE / ENDING). If all three are at or above their floor → OK.',
};
