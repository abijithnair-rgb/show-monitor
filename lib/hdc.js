...
// HDC rate over an explicit [fromDate, toDate] window (inclusive, YYYY-MM-DD),
// from the raw HDC rows: share of qualifying videos that cleared the threshold.
// showId null → across all shows. Returns { pct, hdc, n } (pct null if no rows).
export function windowedHdcRate(hdcRows, showId, fromDate, toDate) {
  if (!hdcRows || !fromDate || !toDate) return { pct: null, hdc: 0, n: 0 };
  const sid = showId == null ? null : String(showId);
  let n = 0, hdc = 0;
  for (const r of hdcRows) {
    if (sid !== null && String(r.show_id) !== sid) continue;
    const pd = String(r.publish_date || '').slice(0, 10);
    if (!pd || pd < fromDate || pd > toDate) continue;
    n += 1;
    if (num(r.HDC_threshold) === 1) hdc += 1;
  }
  return { pct: n ? Math.round((hdc / n) * 1000) / 10 : null, hdc, n };
}
