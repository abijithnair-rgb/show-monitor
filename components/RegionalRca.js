...
// Regional-language (Telugu / Tamil / Malayalam / Kannada) daily RCA cards —
// the label-led HDC/DAU/SR view carried over from the original Daily RCA. Reads
// the LANGUAGE segment rows (segment !== 'hi') for the latest settled report.

const REGIONAL_ORDER = ['te', 'ta', 'ml', 'kn'];
...
// Regional RCA block — te/ta/ml/kn cards for the latest settled report run.
export default function RegionalRca({ rcaRows }) {
  const langRows = useMemo(
    () => (rcaRows || []).filter((r) => r.level === 'LANGUAGE' && r.segment !== 'hi'),
    [rcaRows]
  );
  const reportDate = useMemo(() => {
    const dates = [...new Set(langRows.map((r) => r.report_date).filter(Boolean))].sort().reverse();
    const settled = dates.find((d) => langRows.some((r) => r.report_date === d && isSettled(r.label_is_settled)));
    return settled || dates[0] || '';
  }, [langRows]);

  if (!langRows.length || !reportDate) return null;
  const rows = langRows
    .filter((r) => r.report_date === reportDate)
    .sort((a, b) => REGIONAL_ORDER.indexOf(a.segment) - REGIONAL_ORDER.indexOf(b.segment));
  if (!rows.length) return null;

  return (
    <div className="mt-8">
      <div className="text-sm font-semibold text-slate-500 uppercase tracking-wide mb-2">Regional languages — Telugu / Tamil / Malayalam / Kannada <span className="hint">(report run {reportDate})</span></div>
